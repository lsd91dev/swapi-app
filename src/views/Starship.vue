<template>
  <section class="container container--starship animation-fade-in">
  ...
  <div class="position-relative">
     <div v-if="starship" class="card">

        <div class="card__main animation-fade-in">
          <div class="card__description">
          
          <h1 class="card__name"> {{ starship.name }} </h1>
          
          <h2 class="card__gender"> Model: <small> {{ starship.model }} </small> </h2>
            <div class="card__details"> 
              <p class="card__badge"> 
                <span class="card__badge-title">Cargo capacity:</span> 
                <span class="card__badge-text"> {{ starship.cargo_capacity }}</span> 
              </p>

              <p class="card__badge">
               <span class="card__badge-title"> Crew: </span> 
               <span class="card__badge-text"> {{ starship.crew }} </span> 
              </p>

              <p class="card__badge"> 
                <span class="card__badge-title"> Length: </span> 
                <span class="card__badge-text"> {{ starship.length }} </span> 
              </p>
              <p class="card__badge">
                <span class="card__badge-title"> Passengers: </span> 
                <span class="card__badge-text"> {{ starship.passengers }} </span> </p>
            </div>
          </div>
          <div class="card__image-container">
            <img src="../assets/imgs/millennium-falcon-removebg-preview.png" class="card__image">
          </div>
        </div>
      </div>
     </div>
...
export default {
  ...
  methods: {
    async getStarship(id){
      this.loading = true;
      try {     
        this.starship = await getDataById('starships',id);
