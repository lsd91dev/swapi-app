<template>
  <section class="container container--luke animation-fade-in">
  ...
  <div class="position-relative">
     ...
     <div v-if="character" class="card">
        ...
        <div class="card__main animation-fade-in">
          ...
          <h2 class="card__gender"> Gender: <small> {{ character.gender }} </small> </h2>
            <div class="card__details"> 
              <p class="card__badge"> 
                <span class="card__badge-title">Eye color:</span> 
                <span class="card__badge-text"> {{ character.eye_color }}</span> 
              </p>

              <p class="card__badge">
               <span class="card__badge-title"> Hair color: </span> 
               <span class="card__badge-text"> {{ character.hair_color }} </span> 
              </p>

              <p class="card__badge"> 
                <span class="card__badge-title"> Height: </span> 
                <span class="card__badge-text"> {{ character.height }} </span> 
              </p>
              <p class="card__badge">
                <span class="card__badge-title"> Mass: </span> 
                <span v-if="character.mass" class="card__badge-text"> {{ character.mass }} </span> </p>
            </div>
          </div>
          <div class="card__image-container">
            <img src="../assets/imgs/luke-skywalker-removebg-preview.png" class="card__image">
          </div>
        </div>
      </div>
     </div>
...
export default {
  ...
  methods: {
    async getCharacter(id){
      this.loading = true;
      try {     
        this.character = await getDataById('people',id);
      } catch(error) {
        console.log(error)
...
.container--luke {
  background: url('../assets/imgs/luke-skywalker__wallpaper.jpg');
  background-size:cover;
  background-position: right;
  position:relative;
}
 
.container--luke::before{
  position: absolute;
  content: '';
  top:0;
  bottom:0;
